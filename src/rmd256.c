...
uint8_t Fu8__rmd256(blob_t *ptr_sstc_pssd_blob_input, blob_t *ptr_sstc_pssd_blob_output)
    ...
    */ 
    u64_lcl_number_of_block_in_the_input_message = ptr_sstc_pssd_blob_input->u64_length_data_blob_ / RMD256_BLOCK_SIZE_IN_BYTE;

    /**
    * Check if the modulo of the length of the input message by the rmd256 block size is not zero
    */
    if((ptr_sstc_pssd_blob_input->u64_length_data_blob_ % RMD256_BLOCK_SIZE_IN_BYTE) != 0)
        {
        /**
        ...
        if(u8_lcl_block_carry_padding_flag != FALSE)
            ...
            */ 
            ...
            while(u64_lcl_cnt < RMD256_BLOCK_SIZE_IN_BYTE)
                ...
                */
                if(((u64_lcl_actual_block_number * RMD256_BLOCK_SIZE_IN_BYTE) + u64_lcl_cnt) >= ptr_sstc_pssd_blob_input->u64_length_data_blob_)
                    {
                    /**
                    ...
                    */
                    ...
                    u8_lcl_block_512_bit[u64_lcl_cnt] = ptr_sstc_pssd_blob_input->ptr_u8_data_blob_[(u64_lcl_actual_block_number * RMD256_BLOCK_SIZE_IN_BYTE) + u64_lcl_cnt];
                    }
